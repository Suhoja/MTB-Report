...
mark_new_variants = function(x){
  ...
  for (i in x){
    if(grepl("NEW.*$",i)){
      r = gsub("NEW"," ",i)
      cols=c(cols,paste("\\textcolor{787878}{", formatC(r, dig=2, format="f"), "}",sep=""))
     # cols=c(cols,paste("\\textcolor{787878}{\\underline{", formatC(r, dig=2, format="f"), "}}",sep=""))
    }
    if(!grepl("NEW.*$",i)){
      cols=c(cols,i)
    }
...
col_levels = function(x){
  ...
  for (i in x){
    ...
    if(i=="A1"){ 
      cols=c(cols,paste("\\textbf{\\textcolor{Blue}{", formatC(i, dig=2, format="f"), "}}"))
    }
    if(i=="A2"){
      cols=c(cols,paste("\\textbf{\\textcolor{Cerulean}{", formatC(i, dig=2, format="f"), "}}"))
    }
    if(i=="A3"){
      cols=c(cols,paste("\\textbf{\\textcolor{ProcessBlue}{", formatC(i, dig=2, format="f"), "}}"))
    }
    if(i=="B1"){
      cols=c(cols,paste("\\textbf{\\textcolor{OliveGreen}{", formatC(i, dig=2, format="f"), "}}"))
    }
    if(i=="B2"){
      cols=c(cols,paste("\\textbf{\\textcolor{Green}{", formatC(i, dig=2, format="f"), "}}"))
    }
    if(i=="B3"){
      cols=c(cols,paste("\\textbf{\\textcolor{LimeGreen}{", formatC(i, dig=2, format="f"), "}}"))
    }
  }
 return(cols)
}
...
col_association = function(x){
  cols=c()
  for (i in x){
    if(i=="sensitivity"||i=="sensitivity "||i=="response"||i=="response "||i=="increased benefit"||i=="sensitivity/ response"||i=="sensitivity, resistance"||i=="resistance, sensitivity"){
      cols=c(cols,paste("\\textcolor{Green}{", formatC(i, dig=2, format="f"), "}"))
    }
    if(i=="resistance"||i=="resistance "||i=="no response"||i==" no response"||i=="resistance or non-response"||i=="no sensitivity"||i=="decreased sensitivity"||i=="reduced sensitivity"||i=="adverse response"){
      cols=c(cols,paste("\\textcolor{Red}{", formatC(i, dig=2, format="f"), "}"))
    }
...
ID     <- gsub("_"," ",yaml_input$metadata[["patient_id"]])
maf    <- inputfile
##cnv    <- UPLOAD2()
##tx     <- UPLOAD3()
A      <- table
...
%\hline
\textbf{Patient ID}         & \Sexpr{ID}                   & \textbf{Tissue Type}        &  \Sexpr{yaml_input$metadata[["tissue"]]}    \\
\textbf{Gender}             & \Sexpr{yaml_input$gender}         & \textbf{Tumor Content (\%)} & \Sexpr{yaml_input$tumor_content}    \\ 
\textbf{Disease}            & \Sexpr{cancer}               & \textbf{Number SNVs}        &  \Sexpr{nrow(SNV)}          \\ 
\textbf{Stage}              & \Sexpr{yaml_input$metadata[["stage"]]}       \\   
\textbf{Previous Therapies} & \Sexpr{yaml_input$metadata[["previous_ther"]]}     \\ 
\end{tabular}
\end{table}
...
if (nrow(A)!=0|| nrow(druggableTARGET)!=0){             #issue here, returns nothing
...
colnames(tab_summary_snvs) <- paste("\\textbf{",colnames(tab_summary_snvs),"}")
 ...
 align(y)<-"p{0.1in}p{0.6in}p{2.0in}p{0.8in}"###p{0.7in}p{0.6in}p{0.8in}"
...
A$evidence.col <- col_association(A$Predicts) #checking if function is losing 1 row. rep("o", length(A$Predicts)) This prints ok, so function cuts 1 row for some reason.
...
#Reorganize table
A2 <- A[,c(2,13,4,5,12,7,8,9,11)] #old order A[,c(1,12,3,4,11,6:8,10)]
A2[,2] <- gsub(",",", ", A2[,2]) #formats these to separate rows in table
...
for (drug in names(sort(table(d),decreasing = TRUE))){
  index <- grep(drug,A2[,6],ignore.case = T,fixed=T)
...
#format hlines
hlines <- c(-1, 0, 1, nrow(A2), grep("([A-Z])", A2$Gene))
...
#Second header
A2[nrow(A2)+1,] <- c("\\textbf{Gene}","\\textbf{Variant}","\\textbf{Disease}","\\textbf{Known Variant}","\\textbf{Predicts}",
...
#Format witdh
align(x)<-"p{0.1in}p{0.35in}p{0.85in}|p{0.75in}p{0.8in}p{0.55in}p{1.0in}p{0.55in}p{0.6in}p{0.15in}"
...
\begin{tcolorbox}[colframe=gray!40!white,width=17cm,height=0.7cm,valign=center]
\textbf{\large{SPECIFIC VARIANT-DISEASE CONNECTIONS}}
\end{tcolorbox}

\vspace{0.2cm}
\textbf{ClinVar variant information:} This is a list of the patient's variants which could be found in the NCBI ClinVar database.\footnote{Landrum et al. ClinVar: improving access to variant interpretations and supporting evidence. Nucleic Acids Res . 2018 Jan 4.} These include both germline and somatic variants as well as their related traits. Further information about these variants and related publications can be found at www.ncbi.nlm.nih.gov/clinvar/ by searching for the variant accession numbers. \\
...
#Remove genes from table when duplicated
c = 1
gi = cv[1, "Gene"] # initialize gi with the value in the first row
while (c < nrow(cv)) {
  c = c + 1
  if (cv[c, "Gene"] == gi) {
    cv[c, 1] = " " # replace values with missing data
  } else {
    gi = cv[c, "Gene"]
  }
}
...
if (nrow(cv) !=0){
  ...
  hlines <- c(-1, grep("([A-Z])", cv$Gene) - 1)

  colnames(cv)    <- c("\\textbf{Gene}","\\textbf{Patient's Variant}","\\textbf{Trait}","\\textbf{Pathogenicity}","\\textbf{Evidence}","\\textbf{Accession}")
    
  y<-xtable(cv)
  align(y)<-"p{0.1in}p{0.35in}p{0.8in}|p{1.9in}p{1.2in}p{1.15in}p{0.65in}"
    
  #Print table
  print(y,size="\\fontsize{8pt}{10pt}\\selectfont",floating = FALSE,tabular.environment = "longtable",include.rownames =FALSE,sanitize.text.function = function(x){x},hline.after = hlines, add.to.row=list(pos = list(0), command = "\\hline"))
}
if (nrow(druggableTARGET)==0){
  print("No ClinVar data for variants found")
}
